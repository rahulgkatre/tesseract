...
test "pad" {
    ...
    try std.testing.expectEqualDeep(@TypeOf(out1)._shape, .{ 3, 3, 4, 4 });

    const p2d = comptime .{ .{ 1, 1 }, .{ 2, 2 } };
    const out2 = comptime t4d.pad(p2d, .{ .constant = 0 });
    try std.testing.expectEqualDeep(@TypeOf(out2)._shape, .{ 3, 3, 8, 4 });

    const p3d = comptime .{ .{ 0, 1 }, .{ 2, 1 }, .{ 3, 3 } };
    const out3 = comptime t4d.pad(p3d, .{ .constant = 0 });
    try std.testing.expectEqualDeep(@TypeOf(out3)._shape, .{ 3, 9, 7, 3 });
}

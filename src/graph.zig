...
pub const GraphTensor = struct {
    ...
    zip_fn: *const fn (comptime ptr: *const Self, comptime zip_op: ops.ZipOp, comptime b_ptr: anytype) Self,
    ...
    pub fn zip(comptime self: *const Self, comptime zip_op: ops.ZipOp, comptime b_ptr: anytype) Self {
        return self.zip_fn(self, zip_op, b_ptr);
    }
    ...
    pub fn print_graph(comptime self: *const Self) void {
        ...
        if (self.history != null) {
            switch (self.history.?.op) {
                .MapOp => |op| {
                    const t1 = self.history.?.args.MapOp.a_ptr;
                    std.debug.print("\n\top: {any}\n\tinput:", .{op});
                    t1.print_info();
                    std.debug.print("\n", .{});
                    t1.print_graph();
                ...
                .ZipOp => |op| {
                    const t1 = self.history.?.args.ZipOp.a_ptr;
                    const t2 = self.history.?.args.ZipOp.b_ptr;
                    std.debug.print("\n\top: {any}\n\tinput1:", .{op});
                    t1.print_info();
                    std.debug.print("\n\tinput2:", .{});
                    t2.print_info();
                ...
                .ReduceOp => |op| {
                    const t1 = self.history.?.args.ReduceOp.a_ptr;
